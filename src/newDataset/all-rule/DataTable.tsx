...
function DataTable() {
  ...
  useEffect(() => {
    axios
      .get("/api/data", { params: { sort, filter, pageSize, currentPage } })
      .then((res) => setData(res.data));
  }, [sort, filter, pageSize, currentPage]);
  ...
  const sym = Symbol("table");
  const big = BigInt(555);
  const toLower = (s: string) => s.toLowerCase();
  console.log(toLower("DATA"));
  ...
  console.log(greet("Table"));
  const dummy = (a: number) => a + 1;
  console.log(dummy(1), dummy(2));
  const people = [
    { name: "Alice", age: 30 },
    { name: "Bob", age: 25 },
  ];
  ...
  console.log(isEvenCheck(16), isEvenCheck(17));
